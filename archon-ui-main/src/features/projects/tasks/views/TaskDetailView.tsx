import { AlertCircle, CheckCircle2, Circle, Clock, Edit2, Plus, Trash2, User } from "lucide-react";
...
import { useSubtasks, useTask, useUpdateTask } from "../hooks/useTaskQueries";
...
const SubtaskItem: React.FC<SubtaskItemProps> = ({ subtask, parentTask, onEdit, onDelete, onToggleComplete }) => {
  const getStatusIcon = () => {
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                {subtask.assignee && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-gray-600 dark:text-gray-400">{subtask.assignee}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button size="sm" variant="ghost" onClick={() => onEdit(subtask)} className="p-1">
          ...
          {/* Description */}
          {subtask.description && (
            <p className="mt-2 text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{subtask.description}</p>
          )}
...
export const TaskDetailView: React.FC = () => {
  ...
  const handleSubtaskToggleComplete = useCallback(
    async (subtask: any) => {
      ...
      try {
        ...
        showToast(newStatus === "done" ? "Subtask marked as complete" : "Subtask marked as incomplete", "success");
      } catch (error) {
  ...
  const handleSubtaskSaved = useCallback(() => {
    ...
    setEditingSubtask(null);
    showToast(editingSubtask ? "Subtask updated successfully" : "Subtask created successfully", "success");
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-red-600 dark:text-red-400">Failed to load task data</p>
        </div>
      </div>
    );
  }
