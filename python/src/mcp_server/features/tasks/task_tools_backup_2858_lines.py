...
def register_task_tools(mcp: FastMCP):
    ...
    async def test_basic_health(
        ...
        """Test function copying EXACTLY find_tasks pattern"""
        try:
            api_url = get_api_url()
            timeout = get_default_timeout()

            # EXACT copy of find_tasks pattern
            params = {
                "page": 1,
                "per_page": 10,
                "include_closed": True
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                # EXACT copy of find_tasks normalization
                if isinstance(result, list):
                    tasks = result
                    total_count = len(result)
                elif isinstance(result, dict):
                ...
                # Simple health calculation - NO complex iteration
                task_count = len(tasks)
                health_score = 100 if task_count == 0 else 75  # Simple logic
    ...
    async def find_bottlenecks(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                # Get tasks using exact same pattern as find_tasks
                ...
                tasks_response = await client.get(urljoin(api_url, "/api/tasks"), params=task_params)
                tasks_response.raise_for_status()

                result = tasks_response.json()

                # Normalize response format exactly like find_tasks
                if isinstance(result, list):
                    tasks = result
                elif isinstance(result, dict):
                    if "tasks" in result:
                ...
                if include_dependencies:
                    try:
                        ...
                        # Normalize dependencies response format
